...
const CreatePoint = () => {
  ...
  function handleSelectAddress(event: MouseEvent<HTMLOptionElement>) {
    ...
    axios
      ...
      .then((response) => {
        const addressOptions = response.data.address.full;
        const inputValue = document.getElementById(
          "addressInput"
        ) as HTMLInputElement;
        inputValue.value = response.data.address.full;
        setinitialPosition([
  ...
  function handleInputAddress(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (addressInput.length > 3) {
      axios
        ...
        .then((response) => {
          const addressOptions = response.data.addresses.map(
            (address) => address.a
          );
              ...
              <div>
                {address.map((address) => (
                  <option
                    key={address}
                    value={address}
                    className={address.length === 0 ? "" : "address-option"}
